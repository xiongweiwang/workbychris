...
CREATE USER new_user IDENTIFIED BY password DEFAULT TABLESPACE SYSTEM;
...
--101. Using `CONCAT` to Avoid Division by Zero
...
Compound Queries Examples
--1. Union of Two Queries with Different Columns
SELECT first_name FROM employees WHERE department_id = 10
UNION
SELECT first_name FROM employees WHERE department_id = 20
...
--3. Intersect Two Queries
SELECT first_name FROM employees WHERE department_id = 10
INTERSECT
SELECT first_name FROM employees WHERE salary > 50000;

--4. MINUS Two Queries 最初の問合せには存在し、2番目の問合せには存在しない行
SELECT first_name FROM employees WHERE department_id = 10
MINUS
SELECT first_name FROM employees WHERE department_id = 20;

--5. Self Join with Different Conditions
SELECT e1.first_name AS Employee, e2.first_name AS Manager
FROM employees e1
JOIN employees e2 ON e1.manager_id = e2.employee_id
